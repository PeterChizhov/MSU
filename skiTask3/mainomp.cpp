...
double F_func(double x, double y){
    double cos_pixy = cos(M_PI*x*y);
    double sin_pixy = sin(M_PI*x*y);
    double pix = M_PI*x;
    double piy = M_PI*y;

    return pow(pix, 2.)*(x + y + 4.)*cos_pixy +
            pow(piy, 2.)*(x + y + 4.)*cos_pixy +
            pix*sin_pixy + piy*sin_pixy;
};

double T_edge_func(double x, double y){
    double cos_pixy = cos(M_PI*x*y);
    double sin_pixy = sin(M_PI*x*y);
    return 1 - (4. + x + y)*M_PI*x*sin_pixy + cos_pixy;
};

double B_edge_func(double x, double y){
    double sin_pixy = sin(M_PI*x*y);
    return (4. + x + y)*M_PI*x*sin_pixy;
};

double L_edge_func(double x, double y){
    double sin_pixy = sin(M_PI*x*y);
    return (4. + x + y)*M_PI*y*sin_pixy;
};

double R_edge_func(double x, double y){
    double cos_pixy = cos(M_PI*x*y);
    double sin_pixy = sin(M_PI*x*y);
    return 1 - (4. + x + y)*M_PI*y*sin_pixy + cos_pixy;
...
public:
    ...
    void tofile_plain(std::string path) const {
        std::ofstream approxfile;
        approxfile.open(path, std::ios_base::app);
        for (int i = 0; i < size; ++i)
            approxfile << matrix[i] <<  std::endl;
    }
};
...
class PuassonEquation {
    // метод, пересылающий точки соседним областям
                ...
                if (i == 0) {
                    ...
                    if (j == 0) {
                        ...
                        f_vector(i, j, 0) =
                                F_func(xi, yj) +
                                (2. / h1 + 2. / h2) *
                                (h1 * B_edge_func(xi, yj) +
                                 h2 * L_edge_func(xi, yj)) / (h1 + h2);
                        continue;
                    }
                    //right bottom
                    if (j == M - 1) {
                    ...
                    matrix(i, j, 3) =
                            ...
                            2. * config->alpha_B / h2;
                    f_vector(i, j, 0) =
                            F_func(xi, yj) + (2. / h2) * B_edge_func(xi, yj);
                    continue;
                }
                // top points
                if (i == N - 1) {
                    ...
                    matrix(i, j, 3) =
                            ...
                            2. * config->alpha_T / h2;
                    f_vector(i, j, 0) =
                            F_func(xi, yj) + (2. / h2) * T_edge_func(xi, yj);
                    continue;
                }
                //left points
                if (j == 0) {
                    ...
                    matrix(i, j, 3) =
                            ...
                            2. * config->alpha_L / h1;
                    f_vector(i, j, 0) =
                            F_func(xi, yj) + (2. / h1) * L_edge_func(xi, yj);
                    continue;
                }
                //right points
                if (j == M - 1) {
                    ...
                    matrix(i, j, 3) =
                            ...
                            2. * config->alpha_R / h1;
                    f_vector(i, j, 0) =
                            F_func(xi, yj) + (2. / h1) * R_edge_func(xi, yj);
                    continue;
                }
                // inner points
                {
    ...
    static int optimize(Matrix &matrix, Matrix &f_vector, Matrix &w_vector,
...
public:
    ...
    static void solve(Config *config) {
        ...
        int max_iter;
        max_iter = optimize(matrix, f_vector, w_vector, r_vector, Ar_vector, true_solution,
...
//        double *vec_buf = nullptr;
//        double *straight_vec = nullptr;
//        int size_vec = w_vector.size * config->size_comm_x * config->size_comm_y;
//        if (config->rank == 0) {
//            vec_buf = new double[size_vec];
//            straight_vec = new double[size_vec];
//        }
//        MPI_Gather(&w_vector(0, 0, 0), w_vector.size, MPI_DOUBLE, vec_buf,
//                   w_vector.size, MPI_DOUBLE, 0, config->comm);
        if (config->rank == 0) {
            ...
            std::string path = "outputdir/500x500.csv";
            std::ofstream outputfile;
            outputfile.open(path, std::ios_base::app);
            outputfile << sqrt(error_e2) << ";" << result_time << max_iter << ";" std::endl;

            // w_vector.tofile_plain("outputdir/approximation.csv");

            // true_solution.tofile_plain("truefunc2.csv");
        }
    }
